...
-- 문제6.
-- 평균 연봉이 가장 높은 부서는? 
select a.dept_name as '부서', max(b.avg_salary) as '평균연봉'
from departments a, 
	(select a.dept_name 부서, avg(salary) as avg_salary
	from departments a, dept_emp b, salaries c
	where b.emp_no = c.emp_no
	and a.dept_no = b.dept_no
	and b.to_date = '9999-01-01'
	and c.to_date = '9999-01-01'
        group by de.dept_name) b;

-- 문제7.
-- 평균 연봉이 가장 높은 직책?
SELECT 
    a.title, AVG(a.salary), salary
FROM
    titles a,
    salaries b
WHERE
...
select g.dept_name as '부서이름', concat(a.first_name, ' ', a.last_name) as '이름',max(e.salary) as '연봉', concat(d.first_name, ' ', d.last_name) as '매니저이름', f.salary as '매니저연봉'
  from employees a, dept_emp b, dept_manager c, employees d, salaries e, salaries f, departments g
 where a.emp_no = b.emp_no
   and b.dept_no = c.dept_no
   and c.emp_no = d.emp_no
   ...
   and c.to_date = '9999-01-01'
   and b.to_date = '9999-01-01'
group by g.dept_name, a.first_name, d.first_name, f.salary;
